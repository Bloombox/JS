...
goog.require('proto.bloombox.schema.analytics.context.DeviceOS');
goog.require('proto.bloombox.schema.analytics.context.DeviceRole');
goog.require('proto.bloombox.schema.analytics.context.DeviceScreen');
goog.require('proto.bloombox.schema.analytics.context.NativeDeviceContext');
goog.require('proto.bloombox.schema.analytics.context.OSType');
goog.require('proto.bloombox.schema.analytics.context.PixelSize');
goog.require('proto.bloombox.schema.analytics.context.ScreenOrientation');
...
bloombox.telemetry.buildNativeContext = function() {
  let native = (
    new proto.bloombox.schema.analytics.context.NativeDeviceContext());
  ...
  native.setRole(proto.bloombox.schema.analytics.context.DeviceRole.CLIENT);
  ...
  if (window.screen) {
    ...
    let pixelSizeViewport = (
      new proto.bloombox.schema.analytics.context.PixelSize());
    ...
    let deviceScreen = (
      new proto.bloombox.schema.analytics.context.DeviceScreen());
    ...
    deviceScreen.setOrientation((window.innerHeight > window.innerWidth) ?
      proto.bloombox.schema.analytics.context.ScreenOrientation.LANDSCAPE :
      proto.bloombox.schema.analytics.context.ScreenOrientation.PORTRAIT);
    native.setScreen(deviceScreen);
  }

  // detect OS type and version
  let osType = proto.bloombox.schema.analytics.context.OSType.OS_UNKNOWN;
  ...
  if (goog.userAgent.IPAD ||
    ...
    goog.userAgent.IPOD ||
    goog.userAgent.IOS)
    osType = proto.bloombox.schema.analytics.context.OSType.IOS;
  else if ((goog.userAgent.WINDOWS ||
      goog.userAgent.EDGE_OR_IE) &&
    goog.userAgent.MOBILE)
    osType = proto.bloombox.schema.analytics.context.OSType.WINDOWS_PHONE;
  else if (goog.userAgent.WINDOWS ||
    goog.userAgent.EDGE_OR_IE)
    osType = proto.bloombox.schema.analytics.context.OSType.WINDOWS;
  else if (goog.userAgent.product.ANDROID)
    osType = proto.bloombox.schema.analytics.context.OSType.ANDROID;
  else if (goog.userAgent.MAC)
    osType = proto.bloombox.schema.analytics.context.OSType.MACOS;
  else if (goog.userAgent.LINUX)
    osType = proto.bloombox.schema.analytics.context.OSType.LINUX;

  let osObj = new proto.bloombox.schema.analytics.context.DeviceOS();
